function [T,xval,bestP,bestT,convo,minmet,structure]=Predict_Temp...
...
functions density(), heatcapacity(), and varyingpower()
    ...
    for Tin=Tinvals
        for Pin=Pinvals
            ...
            %the reaction rate 
                        ...
                        T(i)=0.5*(-(rho*cp*epsilon*u*(T(i+1)-T(i-1))/(2*dx)-...
                            pd(i-1)+eta_R*r*H)*dx^2/k+T(i-1)+T(i+1));
                        ...
                        rf=Af*C_CO2(i)*C_H2(i)*exp(-Eaf/(R*T(i))); %mol/m^3 s  HERE
                        rr=Ar*C_CO(i)*C_H2O(i)*exp(-Ear/(R*T(i))); %mol/m^3 s  HERE
                        ...
                        T(i+1)=(-coef*T(i-1)/(2*dx)-k*T(i-1)/dx^2+k*2*T(i)/dx^2-...
                            pd(i-1)+eta_R*H*r)/(k/dx^2-coef/(2*dx));
